...
def process_raw_trajectory(traj):
    """
    Input rows from build_trajectory(): [x_ft, y_ft, speed, lane]
    where x_ft = Local_X (lateral), y_ft = Local_Y (longitudinal).
    Output: [s_m (longitudinal), r_m (lateral), speed_mps, lane_id]
    """
    FT_TO_M = 0.3048
    out = []
    for x_ft, y_ft, spd, lane in traj:
        s_m = float(y_ft) * FT_TO_M      # longitudinal
        r_m = float(x_ft) * FT_TO_M      # lateral
        spd_mps = float(spd) * FT_TO_M   # if spd was ft/s; if already m/s, drop this multiply
        out.append([s_m, r_m, spd_mps, int(lane)])
    return out
