...
#SBATCH --mem=128G
#SBATCH --output=logs/ps_airl_stage1_50veh_%j.out
#SBATCH --error=logs/ps_airl_stage1_50veh_%j.err
...
COLLISION_MODE_SCHEDULE="${COLLISION_MODE_SCHEDULE:-1:200:soft;201:400:mixed;401:${TOTAL_ROUNDS}:full}"
COLLISION_MIXED_ON_FRACTION="${COLLISION_MIXED_ON_FRACTION:-0.5}"
COLLISION_PROXY_PENALTY_COEF="${COLLISION_PROXY_PENALTY_COEF:-1.0}"
POLICY_BC_REGULARIZATION_COEF="${POLICY_BC_REGULARIZATION_COEF:-0.01}"
POLICY_BC_REGULARIZATION_FINAL_COEF="${POLICY_BC_REGULARIZATION_FINAL_COEF:-0.0}"
POLICY_BC_REGULARIZATION_DECAY_ROUNDS="${POLICY_BC_REGULARIZATION_DECAY_ROUNDS:-50}"
...
INITIAL_POLICY_ARGS=()
if [ -n "${INITIAL_POLICY_CHECKPOINT}" ]; then
    if [ ! -f "${INITIAL_POLICY_CHECKPOINT}" ]; then
        echo "INITIAL_POLICY_CHECKPOINT not found: ${INITIAL_POLICY_CHECKPOINT}" >&2
        exit 2
    fi
    INITIAL_POLICY_ARGS=(--initial-policy-checkpoint "${INITIAL_POLICY_CHECKPOINT}")
fi
