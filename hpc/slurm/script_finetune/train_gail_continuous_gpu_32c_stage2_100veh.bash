...
#SBATCH --mem=128G
#SBATCH --output=logs/ps_gail_stage2_100veh_%j.out
#SBATCH --error=logs/ps_gail_stage2_100veh_%j.err
...
export MPLCONFIGDIR="${VFI_LOG_ROOT}/matplotlib_${SLURM_JOB_ID}"
...
mkdir -p "${VFI_LOG_ROOT}"
mkdir -p "${VFI_LOG_ROOT}/simple_ps_gail"
...
RUN_NAME="${RUN_NAME:-ps_gail_stage2_100veh_${SLURM_JOB_ID}}"
ALGORITHM_VARIANT="${ALGORITHM_VARIANT:-gail_bce}"
RESUME_CHECKPOINT="${RESUME_CHECKPOINT:-}"
ALLOW_NON_BEST_RESUME="${ALLOW_NON_BEST_RESUME:-false}"
ACTION_MODE="${ACTION_MODE:-continuous}"
SCENE="${SCENE:-us-101}"
CKPT_ROOT="${CKPT_ROOT:-${VFI_CHECKPOINT_ROOT}}"
CKPT_DATASET="${CKPT_DATASET:-}"
EPISODE_ROOT="${EPISODE_ROOT:-${VFI_HIGHWAY_DATA_ROOT}/processed_20s}"
PREBUILT_SPLIT="${PREBUILT_SPLIT:-train}"
EXPERT_DATA="${EXPERT_DATA:-${VFI_DATA_ROOT}/expert/ngsim_ps_unified_expert_continuous_55145982}"
...
INITIAL_CONTROLLED_VEHICLES="${INITIAL_CONTROLLED_VEHICLES:-50}"
...
CONTROLLED_VEHICLE_SCHEDULE_PROFILE="${CONTROLLED_VEHICLE_SCHEDULE_PROFILE:-gradual}"
CONTROLLED_VEHICLE_SCHEDULE_GRADUAL="${CONTROLLED_VEHICLE_SCHEDULE_GRADUAL:-0:40:50:70;40:80:70:90;80:100:90:100;100:200:100:100}"
...
WARMUP_LEARNING_RATE="${WARMUP_LEARNING_RATE:-5e-5}"
WARMUP_DISC_LEARNING_RATE="${WARMUP_DISC_LEARNING_RATE:-1e-4}"
WARMUP_ENTROPY_COEF="${WARMUP_ENTROPY_COEF:-0.001}"
WARMUP_CLIP_RANGE="${WARMUP_CLIP_RANGE:-0.05}"
...
VEHICLE_INCREASE_WARMUP_ROUNDS="${VEHICLE_INCREASE_WARMUP_ROUNDS:-8}"
...
ROLLOUT_TARGET_AGENT_STEPS_SCHEDULE="${ROLLOUT_TARGET_AGENT_STEPS_SCHEDULE:-0:40:10000:18000;40:80:18000:30000;80:100:30000:40000;100:200:40000:40000}"
PSRO_LITE=true
...
DISC_UPDATES_PER_ROUND="${DISC_UPDATES_PER_ROUND:-1}"
...
POLICY_BC_REGULARIZATION_DECAY_ROUNDS="${POLICY_BC_REGULARIZATION_DECAY_ROUNDS:-0}"
...
VALIDATION_STRESS_EPISODES="${VALIDATION_STRESS_EPISODES:-10}"
...
TEST_EPISODES="${TEST_EPISODES:-30}"
...
ENTROPY_COEF_SCHEDULE="${ENTROPY_COEF_SCHEDULE:-0:50:0.001:0.0005;50:100:0.0005:0.0001;100:200:0.0001:0.0001}"
CLIP_RANGE="${CLIP_RANGE:-0.08}"
CLIP_RANGE_SCHEDULE="${CLIP_RANGE_SCHEDULE:-}"
TARGET_KL="${TARGET_KL:-0.01}"
ABORT_ON_HEALTH_FAILURE="${ABORT_ON_HEALTH_FAILURE:-true}"
COLLISION_MODE_SCHEDULE="${COLLISION_MODE_SCHEDULE:-1:${TOTAL_ROUNDS}:full}"
COLLISION_MIXED_ON_FRACTION="${COLLISION_MIXED_ON_FRACTION:-0.5}"
COLLISION_PROXY_PENALTY_COEF="${COLLISION_PROXY_PENALTY_COEF:-1.0}"
...
DISCRIMINATOR_SPECTRAL_NORM="${DISCRIMINATOR_SPECTRAL_NORM:-false}"
...
if [ "${ABORT_ON_HEALTH_FAILURE}" = "true" ]; then
    ABORT_ON_HEALTH_FAILURE_ARG="--abort-on-health-failure"
else
    ABORT_ON_HEALTH_FAILURE_ARG="--no-abort-on-health-failure"
fi
...
python -m scripts_gail.train_simple_ps_gail \
    ...
    --enable-collision \
    --collision-mode-schedule "${COLLISION_MODE_SCHEDULE}" \
    --collision-mixed-on-fraction "${COLLISION_MIXED_ON_FRACTION}" \
    --collision-proxy-penalty-coef "${COLLISION_PROXY_PENALTY_COEF}" \
