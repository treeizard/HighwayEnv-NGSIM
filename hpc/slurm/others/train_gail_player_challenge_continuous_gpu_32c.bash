#!/bin/bash
#SBATCH --job-name=ps_gail_challenge_32c
#SBATCH --account=bt60
...
#SBATCH --mem=128G
#SBATCH --output=logs/ps_gail_challenge_32c_%j.out
#SBATCH --error=logs/ps_gail_challenge_32c_%j.err
...
export MPLCONFIGDIR="${VFI_LOG_ROOT}/matplotlib_${SLURM_JOB_ID:-manual}"
export MPLBACKEND="${MPLBACKEND:-Agg}"
export PYTHONUNBUFFERED="${PYTHONUNBUFFERED:-1}"
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

cd "${REPODIR}"
mkdir -p "${VFI_LOG_ROOT}/simple_ps_gail" "${MPLCONFIGDIR}"

module load miniforge3
source "$(conda info --base)/etc/profile.d/conda.sh"
conda activate "${VFI_CONDA_ENV:-ngsim_env}"

RUN_NAME="${RUN_NAME:-ps_gail_player_challenge_continuous_gpu_32c_${SLURM_JOB_ID:-manual}}"
...
ENABLE_HARD_EXAMPLE_SELECTION="${ENABLE_HARD_EXAMPLE_SELECTION:-true}"
HARD_EXAMPLE_CANDIDATE_SAMPLES="${HARD_EXAMPLE_CANDIDATE_SAMPLES:-65536}"
HARD_EXAMPLE_SELECTED_FRACTION="${HARD_EXAMPLE_SELECTED_FRACTION:-0.35}"
HARD_EXAMPLE_UNIFORM_MIX="${HARD_EXAMPLE_UNIFORM_MIX:-0.25}"
HARD_EXAMPLE_TEMPERATURE="${HARD_EXAMPLE_TEMPERATURE:-1.0}"
HARD_EXAMPLE_MIN_SAMPLES="${HARD_EXAMPLE_MIN_SAMPLES:-4096}"
...
echo "Hard selector: ${ENABLE_HARD_EXAMPLE_SELECTION} candidates=${HARD_EXAMPLE_CANDIDATE_SAMPLES} selected_fraction=${HARD_EXAMPLE_SELECTED_FRACTION} min=${HARD_EXAMPLE_MIN_SAMPLES}"
...
python -m scripts_gail.train_simple_ps_gail \
    ...
    "$(bool_arg enable-hard-example-selection "${ENABLE_HARD_EXAMPLE_SELECTION}")" \
    --hard-example-candidate-samples "${HARD_EXAMPLE_CANDIDATE_SAMPLES}" \
    --hard-example-selected-fraction "${HARD_EXAMPLE_SELECTED_FRACTION}" \
    --hard-example-uniform-mix "${HARD_EXAMPLE_UNIFORM_MIX}" \
    --hard-example-temperature "${HARD_EXAMPLE_TEMPERATURE}" \
    --hard-example-min-samples "${HARD_EXAMPLE_MIN_SAMPLES}" \
    ...
    --wandb-tags "ps-gail,continuous,player-challenge,hard-selector,psro-lite-${PSRO_LITE},scene-${SCENE}"
