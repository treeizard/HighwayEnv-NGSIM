#!/usr/bin/env python3
"""Measure same-scenario expert replay under policy-evaluation collision physics."""
...
def main() -> None:
    ...
    for domain, scene in domains.items():
        ...
        results[domain] = evaluate_expert_replay_collision_baseline(
            cfg,
            split=str(args.split),
            episodes=int(args.episodes),
            prefix="expert_replay",
        )
    clear_evaluation_worker_caches()
    ...
    payload = {
        "schema_version": 1,
        "audit": "expert_replay_collision_baseline_v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "validation_framework": PAPER_DRIVER_MODEL_VALIDATION_FRAMEWORK,
        "scope": (
            "Same-scenario tracker-action replay under the exact policy "
            "collision backend; this cannot validate surveyed road topology."
        ),
        "episode_root": str(episode_root),
        "split": str(args.split),
        "requested_episodes_per_domain": int(args.episodes),
        "seed": int(args.seed),
        "runtime_contract": {
            "action_mode": "continuous",
