...
def gail_checkpoint_payload(
    *,
    round_idx: int,
    policy: torch.nn.Module,
    discriminator: torch.nn.Module,
    scene_discriminator: torch.nn.Module | None,
    sequence_only_discriminator: bool,
    primary_discriminator_normalizer: tuple[np.ndarray, np.ndarray] | None,
    scene_discriminator_normalizer: tuple[np.ndarray, np.ndarray] | None,
...
def main() -> None:
    ...
    run_root = str(cfg.run_root).strip() or os.path.join("logs", "simple_ps_gail")
    ...
    try:
        ...
        if int(cfg.bc_pretrain_epochs) > 0 and resume_payload is None:
            ...
            atomic_torch_save(
                {
                    **checkpoint_metadata(cfg, method="gail", checkpoint_kind="gail_bc_pretrained"),
                    "round": 0,
                    "policy_state_dict": policy.state_dict(),
                    "expert_metadata": checkpoint_expert_metadata(expert_metadata),
                    "config": vars(cfg),
