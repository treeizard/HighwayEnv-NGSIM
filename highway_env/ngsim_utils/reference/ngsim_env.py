...
class NGSimEnv(AbstractEnv):
    ...
    def __init__(self, config = None):
        ...
        # NEW: Pre-load NGSIM trajectories ONCE
        ego_id  = int(self.config.get("ego_vehicle_ID", 121))
        period  = int(self.config.get("replay_period", 0))
        scene   = self.config.get("scene", "us-101")

        print("[NGSimEnv] Pre-loading NGSIM trajectories (this happens once)…")
        self._trajectory_cache = build_trajectory(scene, period, ego_id)
        print("[NGSimEnv] Done.")
