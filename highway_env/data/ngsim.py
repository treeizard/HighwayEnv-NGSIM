...
from highway_env.data.paras import *
...
class ngsim_data():
  ...
  def read_from_csv(self, filename):
    ...
    with open(filename, 'r', newline='', encoding='utf-8-sig') as f:
        ...
        want_loc = (self.name or "").strip().lower()
        ...
        for row in reader:
          ...
          if len(row) != expected_cols:
              raise ValueError(f"Row {counter+1}: expected {expected_cols} cols, got {len(row)}")
          ...
          # (All columns except the last 'Location' are numeric in your schema)
          norm = row[:]  # copy            

          for i in range(expected_cols):
              if i == loc_idx:
                  continue
              # strip quotes then remove thousand commas
              norm[i] = norm[i].strip().strip('"').replace(',', '')

          # Recreate a CSV-like line if build_from_raw expects a single string
          line_str = ','.join(norm)

          # Optional debug print similar to your original
          if counter % 10000 == 0:
              print(counter)
              # show first few columns for sanity
              preview = ','.join(norm[:6])
              print(preview + ('...' if len(line_str) > 200 else ''))
          ...
          if tmp_vr.unixtime not in self.snap_dict:
              self.snap_dict[tmp_vr.unixtime] = snapshot(tmp_vr.unixtime) # only x,y , lane_ID and vehicle ID right
    ...
    # Final ordering as in your code
    self.snap_ordered_list = sorted(self.snap_dict.keys())
    self.veh_ordered_list = sorted(self.veh_dict.keys())
    for _, tmp_snap in self.snap_dict.items():
        tmp_snap.sort_vehs()
    for _, tmp_veh in self.veh_dict.items():
        tmp_veh.sort_time()
  ...
  def load(self, folder, vr_filename = 'vehicle_record_file.csv', v_filename = 'vehicle_file.csv', snapshot_filename = 'snapshot_file.csv'):
    ...
    self.veh_dict = dict()
    print("LOADING")
    folder += '\\t1118846979700'
    # records
    if not os.path.exists(os.path.join(folder, vr_filename)):
      print(os.path.join(folder, vr_filename))
