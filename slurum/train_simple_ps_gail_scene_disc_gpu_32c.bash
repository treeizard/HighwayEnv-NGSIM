#!/bin/bash
#SBATCH --job-name=ps_gail_scene_32c
#SBATCH --account=bt60
...
#SBATCH --mem=128G
#SBATCH --output=/home/ytao0016/bt60/ytao0016/HighwayEnv-NGSIM/logs/ps_gail_scene_32c_%j.out
#SBATCH --error=/home/ytao0016/bt60/ytao0016/HighwayEnv-NGSIM/logs/ps_gail_scene_32c_%j.err
...
# ---------------------------------------------------------------------------
# Section 2: scene-discriminator training configuration and launch
# ---------------------------------------------------------------------------
...
SCENE_REWARD_COEF="${SCENE_REWARD_COEF:-1.0}"
...
with np.load(files[0], allow_pickle=True) as data:
    missing = {"scene_features", "scene_timesteps"}.difference(data.files)
    if missing:
        raise SystemExit(
            f"{files[0]} is missing {sorted(missing)}; rebuild expert data before scene-discriminator training."
        )
    print("scene expert preflight ok:", files[0], data["scene_features"].shape)
PY
...
python "${REPODIR}/scripts_gail/train_simple_ps_gail.py" \
    ...
    --scene-reward-coef "${SCENE_REWARD_COEF}" \
    ...
    --wandb-tags ps-gail,scene-discriminator,collision,idm,gpu,32c \
    --run-name "${RUN_NAME}"
